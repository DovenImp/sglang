...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def init_backends(self, disable_cuda_graph: bool = False):
        ...
        if self.device == "cuda" or self.device == "musa":
            ...
            # Kernel warmup / flashinfer autotune AND the flashinfer-allreduce
            # workspace pre-init are part of the Runner lifecycle now
            # (BaseRunner.warmup(), run-once): the cuda-graph runners warm up
            # from capture() and the eager runner warms up on its first
            # execute(). No init_backends warmup call — there is no eager
            # fallback here.
    ...
    def _prepare_eager_forward_batch(self, forward_batch: ForwardBatch) -> None:
        ...
        runner's capture/replay, so this is skipped there. Unified into one
        entry so it can later move into the runner's load_batch; the body is the
        previous inline block verbatim.
        """
        # For MLP sync
    ...
    def _forward_raw(
        ...
        with ctx_mgr:
            ...
            # split_prefill is a layer-split mode that does not go through the
            # eager static-buffer path; keep it on ModelRunner.
            ...
            # Prefill cuda graph (piecewise) — only when the prefill runner is a
            # real cuda-graph runner and can run this batch. The EagerRunner's
            # can_run_graph returns False, so this falls through to the eager
            # path below when prefill cuda graph is disabled.
            if (
            ...
            # Eager forward — owned by the EagerRunner (decode / extend / idle).
            eager_runner = self.eager_runner
            if forward_batch.forward_mode.is_decode():
                ret = eager_runner.execute(
                    forward_batch, pp_proxy_tensors=pp_proxy_tensors
                )
            elif forward_batch.forward_mode.is_extend(include_draft_extend_v2=True):
                ret, can_run_graph = eager_runner.execute(
                    forward_batch, pp_proxy_tensors=pp_proxy_tensors
                )
